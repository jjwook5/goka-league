...
3. 사진이 여러 장이면 한 긴 캡처를 위에서부터 겹치게 자른 조각이거나 여러 번 찍은 캡처다. 조각끼리 겹치는 줄이 있으니 한 선수는 한 번만 적어라.
   같은 선수가 서로 다른 값으로 나오면 진행홀이 더 많은 쪽을 써라.
4. 조각의 맨 위·맨 아래 가장자리에서 잘려 보이는 줄은 이웃 조각에 온전히 들어 있다. 잘린 줄 자체는 unreadable 에 적지 말고 무시해라.
5. 모든 사진을 다 본 뒤에도 이름·점수를 확신할 수 없는 선수만 추측하지 말고 unreadable 에 적어라.`;
